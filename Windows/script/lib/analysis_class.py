...
class AnalysisPlot:
    def __init__(self, filepath, filetype, datatype, group_dict, consensus_threshold, progress_bar, model="JC69"):
    ...
    def get_all_distances(self, start_pos, dist_dict=None, multiproc=True):
        ...
        return self.extract_all_distances(temp_mat, str(int(start_pos + (self.window_length / 2))), dist_dict) if not multiproc else temp_mat #else {start_pos: temp_mat}
        #     dist_dict = self.extract_all_distances(temp_mat, str(int(start_pos + (self.window_length / 2))), dist_dict)
        #     return dist_dict
        # else:
        #     return {start_pos: temp_mat}
