...
class RecombinationDetection:
    def __init__(self, dist_dict, global_sim_dict):
        self.recombination_df = None
        self.recombination_df = self.recombination_detect(dist_dict, global_sim_dict)

    def recombination_detect(self, dist_dict, global_sim_dict):
        ...
        result_df = self.filter_dfs(dist_dict, global_sim_dict)
        return result_df

    def filter_dfs(self, dist_dict, global_sim_dict):
        result_list = []
        for refseq, df in dist_dict.items():  # iterate through the distance df by refseq
            ...
            for id, data in df.iterrows():  # iterate through the distances pairs (refseq - id) over all positions
                try:
                    global_sim = global_sim_dict[refseq][id] / 100
                except KeyError:
                    global_sim = global_sim_dict[id][refseq] / 100
