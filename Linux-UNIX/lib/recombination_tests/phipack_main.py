...
class PhiAnalyser:
    ...
    def calc_profile(self, datafile, datatype, align_kind, do_perm=1000, window_size=50, break_window=100, step_size=25, multiproc=True, verbose=False):
        ...
        emp_phi = 0
        if self.Q_output:
            self.Q_output.append(f"Reading sequence file {str(datafile)}")
        taxa_list, alignment, num_sites, num_taxa = extract_fasta(datafile, datatype)
        if self.Q_output:
            self.Q_output.append(f"Allocating space for {num_taxa} taxa and {num_sites} sites.")
        ...
        if validate_alignment(alignment, align_kind, num_taxa, num_sites):
            if self.Q_output:
                self.Q_output.append(f"Alignment looks like a valid {align_kind} alignment")
            num_tests = int(((num_sites - break_window) / step_size) + 1)
            if num_tests < 1:
                if self.Q_output:
                    self.Q_output.append(f"(((num_sites - break_window) / step_size) + 1) must be positive \n"
                ...
                return
            if self.Q_output:
                self.Q_output.append(f"PHI is calculated with w as {window_size}\n")
                self.Q_output.append(f"Checking for recombination using a window size of {break_window} and a step size of {step_size}\n")
            else:
            ...
            for big_counter in range(num_tests):
                if self.Q_output:
                    self.Q_output.append(f"Checking for recombination at {int(big_counter*step_size+break_window/2)} between {big_counter*step_size} and {big_counter*step_size+break_window} ")
                else:
                ...
                if self.Q_output and verbose:
                    self.Q_output.append(f"Estimated diversity is (pairwise deletion - ignoring missing/ambig): {round(divg * 100.0, 1)}%")
                elif verbose:
                ...
                if self.Q_output and verbose:
                    self.Q_output.append(f"Found {num_inf} informative sites.")
                elif verbose:
                ...
                if self.Q_output and verbose:
                    self.Q_output.append(f"using a window size of {window_size} with k as {option_k}")
                elif verbose:
                ...
                if num_inf <= (2 * option_k):
                    ...
                    if self.Q_output and verbose:
                        self.Q_output.append(f"Z-value for PHI is: {format(normal_p_value, '4.6e')}")
                    elif verbose:
                ...
                if self.Q_output and verbose:
                    self.Q_output.append(f"{'=' * 15}")
                    self.Q_output.append(f"\n                      PHI Values")
                    self.Q_output.append(f"                      ----------")
                    self.Q_output.append(f"              Analytical    ({ntrials}) Permutations\n")
                    if valid_normal_approx and do_perm:
                        self.Q_output.append(
                            f"Mean:          {format(mean, '4.2e')}          {format(obs_mean, '4.2e')}")
                    elif valid_normal_approx:
                        self.Q_output.append(f"Mean:          {format(mean, '4.2e')}          --")
                    else:
                        self.Q_output.append("Mean:               --          --")

                    if valid_normal_approx and do_perm:
                        self.Q_output.append(
                            f"Variance:     {format(variance, '4.2e')}          {format(obs_variance, '4.2e')}")
                    elif valid_normal_approx:
                        self.Q_output.append(f"Variance:     {format(variance, '4.2e')}          --")
                    else:
                        self.Q_output.append("Variance:          --          --")
                    self.Q_output.append(
                        f"Observed:    {format(orig_phi, '4.2e')}          {format(orig_phi, '4.2e')}   \n")

                    self.Q_output.append("     **p-Value(s)**     \n       ----------\n")

                if valid_normal_approx:
                    if self.Q_output:
                        self.Q_output.append(f"PHI (Normal):           {format(normal_p_value, '4.2e')}")
                    else:
                        print(f"PHI (Normal):           {normal_p_value}\n")

            ####  different from calc_phi ####
            if self.Q_output:
                self.Q_output.append(f"Number of tests performed is {num_tests}")
            else:
            ...
            if self.Q_output:
                self.Q_output.append("Analysis completed")
            else:
    ...
    def calc_phi(self, datafile, datatype, align_kind, do_perm=1000, window_size= 100, other_stats=True, multiproc=True, verbose=False):
        ...
        if num_taxa <=0 or num_sites <= 0:
            if self.Q_output:
                self.Q_output.append(f"Error. No sequences or sites in alignment")
            raise ValueError("No sequences or sites in alignment")
            return
        ...
        if validate_alignment(alignment, align_kind, num_taxa, num_sites):
            ...
            if num_inf <= (2 * option_k):
                valid_normal_approx = False
                if self.Q_output:
                    self.Q_output.append(f"Too few informative sites to use normal approximation.\nTry doing a permutation test or increasing alignment length\nCan also try decreasing windowsize.")
                else:
                    print("Too few informative sites to use normal approximation.\nTry doing a permutation test or increasing alignment length\nCan also try decreasing windowsize.")
            else:
                valid_normal_approx = True
            ...
            if do_perm:
                ...
                if option_k >= num_inf:
                    ...
                    if self.Q_output:
                        self.Q_output.append("Doing permutation test for PHI")
                    else:
                        print ("Doing permutation test for PHI")
                    if multiproc:
            ...
            if other_stats:
                ...
                if self.Q_output and verbose:
                    self.Q_output.append(f"The Neighbour Similarity score is {format(orig_NSS, '.4e')}")
                else:
            ...
            if self.Q_output and verbose:
                ...
                self.Q_output.append(f"              Analytical    ({ntrials}) Permutations\n")
                if valid_normal_approx and do_perm:
                    self.Q_output.append(f"Mean:          {format(mean, '4.2e')}          {format(obs_mean, '4.2e')}")
                elif valid_normal_approx:
                    self.Q_output.append(f"Mean:          {format(mean, '4.2e')}          --")
                else:
                    self.Q_output.append("Mean:               --          --")
                ...
                if valid_normal_approx and do_perm:
                    self.Q_output.append(f"Variance:     {format(variance, '4.2e')}          {format(obs_variance, '4.2e')}")
                elif valid_normal_approx:
                    self.Q_output.append(f"Variance:     {format(variance, '4.2e')}          --")
                else:
                    self.Q_output.append("Variance:          --          --")
                self.Q_output.append(f"Observed:    {format(orig_phi, '4.2e')}          {format(orig_phi, '4.2e')}   \n")
            ...
            if other_stats:
                if self.Q_output:
                    self.Q_output.append("     **p-Value(s)**     \n       ----------\n")
                    val = emp_NSS / ntrials
                    self.Q_output.append(f"NSS:                         {format(val, '4.2e')}  ({ntrials} permutations)")
                    val = emp_maxchi / ntrials
                    self.Q_output.append(f"Max Chi^2:             {format(val, '4.2e')}  ({ntrials} permutations)")
            ...
            if do_perm:
                if self.Q_output:
                    val = emp_phi / ntrials
                    self.Q_output.append(f"PHI (Permutation):   {format(val, '4.2e')}  ({ntrials} permutations)")
                else:
                    val = emp_phi / ntrials
            ...
            if self.Q_output:
                self.Q_output.append(f"{align_kind} alignment is not valid.")
    ...
    def print_maxchi_output(self, result_tuple, num_poly_unambig, maxchi_window, taxa_list):
        ...
        if self.Q_output:
            self.Q_output.append(f"Number of umabiguous polymorphic sites is {num_poly_unambig} \n")
            self.Q_output.append(f"Window size is {maxchi_window} polymorphic sites")
            self.Q_output.append(f"Best breakpoint for Max Chi found with sequences {taxa_list[taxa_a]} and {taxa_list[taxa_b]}. r and s are {best_r} and {best_s}")
            self.Q_output.append(f"Value of maximum breakpoint is: {best_val}")
            self.Q_output.append(f"Coordinates of breakpoint with only polymorphic sites (start,breakpoint,end) = ({start_poly}, {breakpoint_poly}, {end_poly})")
            self.Q_output.append(f"Coordinates of breakpoint with all sites (start,breakpoint,end)=({start_all}, {breakpoint_all}, {end_all}) \n")
        else:
    ...
    def inc_stats(self, max_size, num_chars, inc_matrix, verbose=False):
        ...
        if verbose:
            if self.Q_output:
                self.Q_output.append(f"Distribution of scaled incompatibility scores:\n")
                self.Q_output.append(f"score (%)")
                if total != 0:
                    for i in range(act_max + 1):
                        ...
                        for j in range(int(prop)):
                            o_str = o_str + "o"
                        if self.Q_output:
                            self.Q_output.append(f"{i} ({round(prop * 2, 1)}): {o_str}")
            else:
